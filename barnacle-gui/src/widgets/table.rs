use iced::{
    Alignment, Background, Element, Event, Length, Pixels, Rectangle, Size, Vector,
    advanced::{self, Clipboard, Layout, Shell, Widget, layout, mouse, overlay, renderer, widget},
    alignment,
};
    ...
    Renderer: advanced::Renderer,
{
    ...
    fn tag(&self) -> widget::tree::Tag {
        widget::tree::Tag::of::<LayoutState>()
    }

    fn state(&self) -> widget::tree::State {
        widget::tree::State::new(LayoutState {
    ...
    fn layout(
        ...
        let layout_state = tree.state.downcast_mut::<LayoutState>();
        ...
        layout_state.column_widths = vec![0.0; self.columns.len()];
        layout_state.row_heights = vec![0.0; rows];
        ...
        for (i, (cell, state)) in self.cells.iter_mut().zip(&mut tree.children).enumerate() {
            ...
            if column == 0 {
                ...
                if row > 0 {
                    y += layout_state.row_heights[row - 1] + spacing_y;

                    if row_factor != 0 {
                        total_fluid_height += layout_state.row_heights[row - 1];
            ...
            layout_state.column_widths[column] = layout_state.column_widths[column].max(size.width);
            layout_state.row_heights[row] = layout_state.row_heights[row].max(size.height);
            cells[i] = layout;
        ...
        // SECOND PASS
        // Lay out fluid cells, using layout state from the first pass as limits
        let left = Size::new(
            available.width
                - layout_state
                    .column_widths
        ...
        for (i, (cell, state)) in self.cells.iter_mut().zip(&mut tree.children).enumerate() {
            ...
            let max_width = if width_factor == 0 {
                if size.width.is_fill() {
                    layout_state.column_widths[column]
                } else {
        ...
        for (i, cell) in cells.iter_mut().enumerate() {
            ...
            x += layout_state.column_widths[column] + spacing_x;
        }
    ...
    fn draw(
        ...
        let layout_state = tree.state.downcast_ref::<LayoutState>();
        ...
        if self.separator_x > 0.0 {
            ...
            for width in
                &layout_state.column_widths[..layout_state.column_widths.len().saturating_sub(1)]
            {
        ...
        if self.separator_y > 0.0 {
            ...
            for height in
                &layout_state.row_heights[..layout_state.row_heights.len().saturating_sub(1)]
            {
